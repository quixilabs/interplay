...
export default function TensionHeatmap({ data }: TensionHeatmapProps) {
    ...
    if (!tensionAnalysis || !tensionAnalysis.crossDomainAnalysis) {
        return (
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-center mb-4">
                    <Zap className="h-5 w-5 text-sage mr-2" />
                    <h3 className="text-lg font-semibold text-slate-900">Tension Heatmap</h3>
                    <Info className="h-4 w-4 text-slate-400 ml-2" />
                </div>
                <div className="text-center py-8">
                    <p className="text-slate-500">No tension data available. Students need to complete the tensions assessment section to generate this analysis.</p>
                </div>
            </div>
        );
    }
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-slate-900">Tension Heatmap</h3>
                            <p className="text-sm text-slate-600">Where student needs and enablers don't match</p>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-sm text-slate-500">Based on {tensionAnalysis.totalTensionResponses} tension assessments</div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    {heatmapData.filter((t: any) => t.avgGapScore >= 0.2).length === 0 && (
                        <div className="text-center py-4 text-slate-500">
                            ...
                            <p>Great news! No critical tension gaps identified in current data.</p>
                            <p className="text-sm mt-1">Students appear to have good balance between their needs and available support.</p>
                        </div>
                    )}
                </div>
            </div>
